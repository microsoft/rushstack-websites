...
export async function loadPosts(options: ILoadPostsOptions): Promise<IPostJson[]> {
  ...
  // Fetch posts. Count comes from `data-post-limit` attribute.
  const limit: number = options.postLimit ?? 5;
  // Whether replies are filtered comes from `data-exclude-replies` attribute.
  const excludeReplies: boolean = options.excludeReplies ?? false;
...
interface IMastotdonPostProps {
  post: IPostJson;
  userUrl: string;
}

export function MastotdonPost(props: IMastotdonPostProps): JSX.Element {
  ...
  const statusPrepend: JSX.Element | undefined = outerPost.reblog ? (
    <div className={styles.statusPrepend}>
      ...
      <div>
        <a
          className={styles.displayName_account}
          target="_blank"
...
export function MastodonFeed(props: IMastodonFeedProps): JSX.Element {
    ...
    (async () => {
      try {
        const posts: IPostJson[] = await loadPosts({
          userUrl,
          excludeReplies: props.excludeReplies,
          postAccountId: props.postAccountId,
          postLimit: props.postLimit
        });
  ...
  }, [props.mastodonUserFullName, props.excludeReplies, props.postLimit, props.postAccountId]);
  ...
  return (
    <div className={[styles.feed, 'no-external-link-icon'].join(' ')}>
      ...
      {posts.map((post, index) => (
        <MastotdonPost key={index} post={post} userUrl={userUrl} />
      ))}
    </div>
  );
}
