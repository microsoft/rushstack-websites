...
[Home](./index.md) &gt; [@rushstack/node-core-library](./node-core-library.md) &gt; [Async](./node-core-library.async.md) &gt; [mapAsync](./node-core-library.async.mapasync_1.md)
...
This API is similar to the system `Array#map`<></>, except that the loop is asynchronous, and the maximum number of concurrent units can be throttled using [IAsyncParallelismOptions.concurrency](./node-core-library.iasyncparallelismoptions.concurrency.md)<></>. Using the [IAsyncParallelismOptions.weighted](./node-core-library.iasyncparallelismoptions.weighted.md) option, the weight of each operation can be specified, which determines how many concurrent units it takes up.
