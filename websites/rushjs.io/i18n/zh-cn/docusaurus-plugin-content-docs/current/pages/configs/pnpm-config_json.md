...
> NOTE: This config file was introduced with Rush 5.79.0. Prior to that release,
> PNPM settings were instead stored in the `"pnpmOptions"` section of **rush.json**.
> For backwards compatibility, Rush 5 still accepts the `"pnpmOptions"` section.
> If you are upgrading an old monorepo, in order to access these new PNPM settings,
> you must manually delete the `"pnpmOptions"` setting from **rush.json** and
> create the **pnpm-config.json** file.
